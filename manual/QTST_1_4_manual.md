...
双分子反应，温度$T=298\ \mathrm{K}$，生成过渡态的势垒$\Delta U=57.95\ \mathrm{kJ/mol}$，过渡态配分函数$Q_{\mathrm{TS}}=8.48\times10^9$，反应物$\mathrm{A}$配分函数$Q_{\mathrm{A}}=1.12\times10^9$，反应物$\mathrm{B}$配分函数$Q_{\mathrm{B}}=8.05\times10^4$，转动配分函数均经过转动对称简并校正，不考虑量子隧穿效应。
