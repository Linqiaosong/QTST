...
{
    std::cout<<"====================================================="<<std::endl;
    std::cout<<"\t  QD v1.2 量子动力学计算软件"<<std::endl;
...
:Temperature(temperature),VirtualFreq(virtualFreq)  //初始化列表
{
    ...
    std::cout<<"温度κ="<<Temperature<<" K"<<std::endl;
    std::cout<<"虚频ν="<<-VirtualFreq<<" cm-1"<<std::endl;
    std::cout<<"隧穿势垒ΔU=nan"<<std::endl;
...
:Temperature(temperature),VirtualFreq(virtualFreq),Barrier(barrier),V(v) //初始化列表
{
    ...
    std::cout<<"温度κ="<<Temperature<<" K"<<std::endl;
    std::cout<<"虚频ν="<<-VirtualFreq<<" cm-1"<<std::endl;
    std::cout<<"隧穿势垒ΔU="<<Barrier<<" kJ/mol"<<std::endl;
...
double QTunnel::Skodje_Truhlar() const
{
    ...
    if(Barrier==-1.0) //近似的Skodje-Truhlar方法
    {
        if(Beta>Alpha)
        {
            std::cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
            std::cout<<"!!"<<std::endl;
            std::cout<<"!!\t未提供隧穿势垒ΔU和放热方向校正系数V的数值，通过近似的Skodje-Truhlar方法计算!!"<<std::endl;
            std::cout<<"!!"<<std::endl;
            std::cout<<"!!\tAlpha<Beta，可能造成重大错误\t\t\t\t!!"<<std::endl;
            std::cout<<"!!"<<std::endl;
            std::cout<<"!!\t建议补充隧穿势垒ΔU和放热方向校正系数V的数值，进行完整的Skodje-Truhlar方法计算!!"<<std::endl;
            std::cout<<"!!"<<std::endl;
            std::cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
        }
        return approximate; 
...
:Dynamic(temperature,sigma,kapa,freeEnergy) 
{
    ...
    std::cout<<"生成过渡态的Gibbis自由能ΔG="<<FreeEnergy<<" kJ/mol"<<std::endl;
...
:Dynamic(temperature,sigma,kapa,barrier,partitionFunction_TS),PartitionFunction_A(partitionFunction_A)
{
    ...
    std::cout<<"反应路径简并数σ="<<Sigma<<std::endl;
    std::cout<<"量子隧穿效应透射系数κ="<<Kapa<<std::endl;
    std::cout<<"生成过渡态的Gibbis自由能ΔG=nan"<<std::endl;
